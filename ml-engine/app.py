...
def process_session():
    ...
    # 4. Trigger R Feature Engineering
    ...
    # 5. Trigger PySpark Inference
    print("Running PySpark ML Pipeline...")
    spark_result = subprocess.run(['python3', 'spark_model.py'], capture_output=True, text=True)
    if spark_result.returncode != 0:
        return jsonify({"error": "Spark script failed", "details": spark_result.stderr}), 500

    # 6. Read final output and return to Node
    try:
